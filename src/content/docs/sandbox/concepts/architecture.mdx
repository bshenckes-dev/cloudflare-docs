...
2. **Durable Object** routes to container using HTTP or WebSocket transport
...
The SDK supports two transport mechanisms between the Durable Object and container:

- **HTTP (default)** - Each SDK operation creates a new HTTP request. Simple and easy to debug.
- **WebSocket** - Multiplexes all operations over a single persistent connection. Reduces sub-request count for high-frequency operations.

Configure transport using the `transport` option in [Sandbox options](/sandbox/configuration/sandbox-options/). The SDK handles all transport-level details automatically - your code works identically with both transports.
