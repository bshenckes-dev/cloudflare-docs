...
Control how the SDK communicates with containers. The SDK supports two transport protocols:

- **HTTP** (default) - Each SDK operation (file read, command execution, etc.) creates a separate HTTP request to the container
- **WebSocket** - All SDK operations are multiplexed over a single persistent WebSocket connection

**When to use WebSocket transport**:

WebSocket transport is designed for Workers and Durable Objects that make many SDK calls and need to avoid sub-request limits. Each HTTP request to the container counts as a sub-request, which can quickly exhaust the 1000 sub-requests per Worker invocation limit.

With WebSocket transport:
- All operations share a single WebSocket connection (one sub-request total)
- File operations, command execution, and process management are multiplexed over this connection
- The connection persists for the lifetime of the sandbox, reducing overhead
...
<TypeScriptExample>
```ts
// Worker code remains unchanged - transport is configured via environment variable
const sandbox = getSandbox(env.Sandbox, 'user-123');

// These operations now use WebSocket instead of HTTP
await sandbox.writeFile('/app/data.json', JSON.stringify(data));
await sandbox.exec('python process.py');
const result = await sandbox.readFile('/app/output.json');
```
</TypeScriptExample>

:::note[Transparent to your code]
WebSocket transport is a drop-in replacement for HTTP transport. No code changes are required - simply set the environment variable and redeploy your Worker. All SDK methods work identically with both transports.
:::

**Use HTTP transport when**:
- Making fewer than 50-100 SDK calls per Worker invocation
- You need maximum compatibility (HTTP is the proven default)
- Debugging issues (HTTP requests are easier to inspect in logs)

**Use WebSocket transport when**:
- Making hundreds of SDK calls per Worker invocation
- Running into sub-request limit errors
- Building applications with heavy file I/O or many command executions
